  ...
  class Parser {
    ...
    getResult() {
      ...
      for (let i = 0; i < tr_list.length; i ++) {
        ...
        let _sub_title = _sub_title_tag ? (_sub_title_tag.attr('title') || _sub_title_tag.text()) : '';
        ...
        let _download_url = site.url + details_link.replace('details','download') + "&trackerssl=1";

        // 定位种子大小，做种和优惠tag
        let _size_peer_block = torrent_data_raw.find('.rowfollow.vcenter.nowrap');
        ...
        let _completed_tag = torrent_data_raw.find("a[href^='viewsnatches.php?id=']");
        ...
        let _date_tag = torrent_data_raw.find("div.small").filter(function () {
          return /(\d{4}-\d{2}-\d{2}[^\d]+?\d{2}:\d{2}:\d{2})/.test($(this).html());
        });
        ...
        let _category = torrent_data_raw.find('div.category_text').text().trim();
        ...
        let data = {
          ...
          completed: _completed_tag ? _completed_tag.text().replace(',', '') : 0,
          comments: _comments,
          site: site,
